...
class TalkController extends CommonController
{
    ...
    public function add ()
    {
        if (IS_POST) {
            ...
            if ($model->create()) {
                ...
                $model->talk_image = empty($_POST['img_url'][0]) ? '' : dhtmlspecialchars(
                        $_POST['img_url'][0]);
    ...
    public function edit ()
    {
        ...
        if (IS_POST) {
            ...
            $brand_temp = $brand_model->getBrandList($map, 
                    'brand_id,brand_name');
            $brand = [];
            ...
            $talk_info['talk_content'] = html_entity_decode(
                    $talk_info['talk_content']);
    ...
    public function comment ()
    {
        ...
        foreach ($data['list'] as $k => $v) {
            $data['list'][$k]['talk_title'] = M('talk')->where(
                    [
                            'talk_id' => $v['talk_id']
                    ])->getField('talk_title');
        }
