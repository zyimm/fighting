	...
	<table class="table table-bordered table-hover table-striped">
		<tbody> 
			...
			<?php if(is_array($activity_list)): $i = 0; $__LIST__ = $activity_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$activity): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php echo get_store_name($activity['store_id']);?></td>
				...
				<td>
					-x-x-x-
					<!-- <a href="/Admin/Activity/edit?activity_id=<?php echo ($activity["activity_id"]); ?>" class="button bg-blue button-little">编辑</a>
					<?php if($activity["status"] == 1 ): ?><a href="/Admin/Activity/apply?activity_id=<?php echo ($activity["activity_id"]); ?>" class="button bg-main button-little">查看报名列表</a>
					<?php else: ?>
					<a href="javascript:alert('该活动处于关闭状态!')" class="button bg-gray button-little">查看报名列表</a><?php endif; ?>
					<a href="javascript:void(0)" onclick="itemDel(<?php echo ($activity["activity_id"]); ?>,'/Admin/Activity/del');" class="button bg-dot button-little">删除</a> -->
				</td>
