...
<div class="padding table-responsive">
	<?php echo crumbs(23,'编辑/添加品牌');?>

	<hr />
	<form method="post"  action='/Admin/Brand/editAdmin' class="form-x"  >
		<input type="hidden" name='admin_id' value='<?php echo ($admin_info["id"]); ?>' />
		<table class="table"> 
			<tr>
				<td>选择品牌:</td>
				<td>
					<select  class='input' id='brand_id' name='brand_id'  onchange=''   style=''  ><?php  foreach($brand as $key=>$val) { if(!empty($admin_info['brand_id']) && ($admin_info['brand_id'] == $key || in_array($key,$admin_info['brand_id']))) { ?><option selected="selected" value="<?php echo $key; ?>"><?php echo $val; ?></option><?php }else { ?><option value="<?php echo $key; ?>"><?php echo $val; ?></option><?php } } ?></select>
				</td>
			</tr>
			<tr>
				<td>管理员名称:</td>
			...
			callback:function(data){
				if(data.status==1){
					layer.msg(data.info,{time: 3000,icon:1},function(){
						layer.closeAll();
						window.location.href='/Admin/Brand/brandAdminList.html';
					});	
				}else{
