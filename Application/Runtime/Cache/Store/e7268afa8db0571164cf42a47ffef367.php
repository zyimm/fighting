...
<div class="padding table-responsive">
	<?php echo crumbs(13,'编辑','store_menu');?>
	<hr />
	<form method="post"  action='/Store/Ad/add' class="form-x"  enctype="multipart/form-data">
		...
		<table class="table table-hover" style=''>
			...
			<tr>
				...
				<td class="text-left"><input name='ad_title' id='_id' datatype='*2-48'    style="width:60%" class='input  display-inline-block' type='text' value='<?php echo ($ad_info['ad_title']); ?>'  required='required'   placeholder= '请认真填写此项!'></td>
			</tr>
			...
			<tr> 	
				<td>跳转类别:</td>
				<td style="text-align:left">
					<input name="ad_type" type="radio" value=1 <?php if($ad_info["ad_type"] == 1 ): ?>checked="checked"<?php endif; ?> />广告链接
					<input name="ad_type" type="radio" value=2 <?php if($ad_info["ad_type"] != 1 ): ?>checked="checked"<?php endif; ?> />详情页面
				</td>
			</tr>
			...
			<tr>
				<td>链接</td>
				<td class="text-left">
					<input name='ad_url' id='_id' datatype='url'    style="width:60%" class='input display-inline-block' type='url' value='<?php echo ($ad_info['ad_url']); ?>'  required='required'   placeholder= '请认真填写此项!'>
				</td>
			...
			<tr>
				...
				<td>
					 <!-- 加载编辑器的容器 -->
   					<script id='editor' name='content' type='text/plain'><?php echo ($ad_info['content']); ?></script>
				    <!-- 配置文件 -->
			...
			<tr> 	
				<td>是否显示:</td>
				<td style="text-align:left">
					<input name="status" type="radio" value=1 <?php if($ad_info["status"] == 1 ): ?>checked="checked"<?php endif; ?> />显示
					<input name="status" type="radio" value=0 <?php if($ad_info["status"] == 0 ): ?>checked="checked"<?php endif; ?> />隐藏	
				</td>
			</tr>
			...
			callback:function(data){
				if(data.status==1){
					layer.msg(data.info,{time: 3000},function(){
						layer.closeAll();
						window.location.href='/Store/Ad/index.html';
					});	
				}else{
					layer.msg(data.info);
				}
