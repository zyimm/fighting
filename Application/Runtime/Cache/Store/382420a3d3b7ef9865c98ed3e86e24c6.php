	...
	<table class="table table-bordered table-hover table-striped">
		<tbody> 
			...
			<?php if(is_array($exam_list)): $i = 0; $__LIST__ = $exam_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$exam): $mod = ($i % 2 );++$i;?><tr>
				...
				<td>
					<a href="/Store/Course/edit?exam_id=<?php echo ($exam["exam_id"]); ?>" class="button bg-blue button-little">编辑</a>
					<?php if($exam["status"] == 1 ): ?><a href="/Store/Course/apply?exam_id=<?php echo ($exam["exam_id"]); ?>" class="button bg-main button-little">查看报名列表</a>
					<?php else: ?>
					<a href="javascript:alert('该考试处于关闭状态!')" class="button bg-gray button-little">查看报名列表</a><?php endif; ?>
					<a href="javascript:void(0)" onclick="itemDel(<?php echo ($exam["exam_id"]); ?>,'/Store/Course/del');" class="button bg-dot button-little">删除</a>
				</td>
