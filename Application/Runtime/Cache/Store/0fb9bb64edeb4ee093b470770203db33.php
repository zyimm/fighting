		...
		<div class="panel-body">
			<form method="post" action='' class="form-x"  />
				...
				<table class="table"> 
					<tr>
						<td>角色名称:</td>
						<td class="text-left"><input name='title' id='_id' datatype='*'     class='input width-250 display-inline-block ' type='text' value='<?php echo ($role_info["title"]); ?>'  required='required'   placeholder= '角色名称'></td>
					</tr>	
					<tr> 	
						<td>角色状态:</td>
		...
		$(".form-x").Validform({	
			...
			callback:function(data){
				if(data.status==1){
					...
					layer.closeAll();
					window.location.href='/Store/Role';
				});	
				}else{
					layer.msg(data.info);
				}
